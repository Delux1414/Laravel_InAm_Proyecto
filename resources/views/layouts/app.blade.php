...
{{--Vamos a verificar si un usuario esta autenticado--}} 
    @auth()
        <nav class="flex gap-4 items-center">
            ...
            <a class="font-bold uppercase text-[#00324D] hover:text-[#38A800]" href="{{ route('post.index', auth()->user()->username) }}">
                Hola <span class="font-bold" > {{ auth()->user()->username }} </span>
            </a>

            <form action=" {{ route('logout') }} " method="post">
                @csrf
                <button type="submit" class="font-bold uppercase  text-[#00324D] hover:text-[#38A800]" >
                    Cerrar sesión
                </button>
            </form>
    ...
    @guest()
        <nav class="flex gap-5 items-center">
            <a class="font-bold uppercase text-[#00324D] hover:text-[#38A800] " href="{{ route('login') }}">Login</a>
            <a class="font-bold uppercase text-[#00324D] hover:text-[#38A800]" href="{{route('register')}}">
                Crear cuenta
            </a>
        </nav>
    @endguest
